...
pub struct Guard<'srv, D: Database> {
    server: &'srv Server<D>,
...
impl<'srv, D: Database> Drop for Guard<'srv, D> {
    fn drop(&mut self) {
...
fn encode_baton<D: Database>(server: &Server<D>, stream_id: u64, baton_seq: u64) -> String {
...
pub async fn run_expire<D: Database>(server: &Server<D>) {
    future::poll_fn(|cx| {
        let mut state = server.stream_state.lock();
